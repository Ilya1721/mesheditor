...
namespace RenderSystem
{
	...
	void BasicRenderer::setMVP(const float* mvp) noexcept
	{
		glUniformMatrix4fv(mMVP, 1, false, mvp);
	}
	...
	int BasicRenderer::loadShader(const std::string& shaderPath, int shaderType) noexcept
	{
		...
		if (!isCompiled)
		{
			auto shaderLog = std::move(getShaderLog(shader, SHADER_LOG_TYPE::SHADER));
