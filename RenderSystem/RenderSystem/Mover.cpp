...
{
  Mover::Mover(Window* window) : Operation(window) {}
  ...
  ) const
  {
    ...
    if (mWindow->getProjectionType() == PROJECTION_TYPE::PERSPECTIVE)
    {
      return glm::translate(
        mWindow->projectToCameraTargetPlane(endPosInWorldSpace) -
        mWindow->projectToCameraTargetPlane(startPosInWorldSpace)
      );
    }
